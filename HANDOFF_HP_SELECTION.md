...
regenerated by `scripts_bnn/round5_disclosures.py`.
...
- the rule-winner vs best-of-all vs eligible-best comparison for each (§3.1: *always
  ...
  owed there yet;
- the item-C plateau side effect (§4.3.125 §3, §4.3.126 §3): medium_diverse spent
  18 trials locked on a width **8% worse** than its neighbour because `J`
  saturated at `log 2`. This belongs in §7.4 with the round-5 results, not in
  §7.1.
