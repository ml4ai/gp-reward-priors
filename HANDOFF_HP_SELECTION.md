...
| **PT large_play** | 0.2100 | **0.4039** | **+0.1244** | 22/45 |
   ...
   **22–49% of trial pairs change order**, so the change alters which
   ...
   > 🔁 **SUPERSEDED (2026-09-24, §7.5 B).** This table came from a mid-sweep
   > subsample: 10 trials per sweep, 6 of 8 sweeps. It was recomputed on all 283
   > finished runs of the completed sweeps. **The winner changes in all 8
   > sweeps**, measured directly, where this section inferred it from pair
   > flips. In two sweeps the old rule's pick ranks #62 of 64 and #61 of 66 on the
   > honest objective. Quote §7.5 B, not this table.
...
#### DECISION (2026-09-18, at the user's direction): CENTRE, in both paths

An earlier draft of this section recommended deferring to a future round. **That
was wrong and is withdrawn.** Deferring does not avoid the risk, it picks the
`raw` branch by default and without deciding — and if raw turns out to be wrong,
round 5 has to be re-run anyway. The question is decidable now, on the evidence
and the argument already in hand.

**It is not a metric preference. It is a question about whether the method's
mechanism is real**, and that has to be settled *before* a round whose entire
purpose is to select a configuration that delivers the mechanism.

**The argument, in three steps:**

1. **The paper's mechanism requires a STATE-DEPENDENT penalty.** CVaR helps
   because it penalises poorly-determined states more. Write
   `depth(s,a) = mean_j f_j(s,a) − CVaR_j f(s,a)`. If the per-draw offset
   dominates the sort, the same draws are selected everywhere, `depth` goes
   near-constant, and **the mechanism is inert** — the reward is
   `mean − constant`.
2. **§4.3.113 measured that the offset DOES dominate**, on every run tested
   (ratio shifts +37% to +235%; the `n_meas` 26 control's CVaR reward reads
   `log 2` under centring). So raw CVaR is currently *not* delivering the
   mechanism.
3. **The offset should not be penalised anyway, and this is a proof rather than
   a preference.** It is unidentified by the likelihood (§4.3.10) — §3.6.3
   already excluded it from the stationarity gate on that ground — **and
   §5.2's gauge overwrites the level deterministically downstream.** You cannot
   be conservative about a quantity you then set by fiat.

**What centring does NOT disturb** — checked, not assumed:

| | effect of centring |
|---|---|
| the posterior **mean** reward | changes by a constant only (`mean_j c_j`), which the gauge removes — **the mean-reward baseline and the MR/PT comparison are untouched** |
| mean CE, predictive CE, accuracy | Class A, **exactly** invariant (§4.3.61) |
| gate 1 | already centred — for the first time it becomes *consistent* with the objective |
| gate 2 | the gap stops being suppressed, so it passes more often — **this is gate 2 finally measuring what §3.2.6 designed it to measure**, not a loosening |

**Comparability requirement.** `qlearning_dataset_mr_ensemble` takes CVaR over
snapshots with the identical `partitioned[:n_tail].mean(axis=0)` structure, so
**MR-ensemble-CVaR must be centred too**, or §3.1's cross-family comparability
breaks. Same one-line operation, applied in both places.

#### The one free check before it is wired — and it can overturn more than this

`--cvar-ce` computes the centred column from saved chains at zero training
compute, and **ten runs now have chains**: the four §4.3.109 arms and the six
§4.3.112 ladder rungs, spanning sensible configurations to blown-up ones.

> ⚠️ **The check can produce a result far more serious than "use raw instead".**
> §4.3.113 measured the `n_meas` 26 control — one of the better configurations on
> the raw objective — at centred CVaR CE **0.6928 ≈ log 2**. If that holds across
> the ten runs, it says **the conservative reward has no predictive content on
> the identified component at all**: the whole apparent conservatism is a global
> shift the gauge discards. That is a finding about the **method**, not the
> metric, and it would be far better known before round 5 than after.

**Pre-registered reading, fixed before the check runs:**

- **At least one configuration's centred CVaR CE below `log 2` by more than its
  2·SE** ⇒ the centred objective has predictive content. **Wire centring into
  both paths and proceed with round 5.**
- **Every run at or above `log 2`** ⇒ do **not** quietly fall back to raw, whose
  advantage §4.3.113 showed to be the suppression artefact. **Stop and report**:
  the mechanism is not demonstrable at any configuration reached so far, and
  round 5 cannot select for something that is not there. That is a §3.2.9-style
  disclosure, not a reason to re-tune.
- Also record whether the centred objective **discriminates** — spread across the
  ten runs against their jackknife SEs — since an objective that cannot separate
  configurations cannot drive a sweep (§3.2.3's problem).

Run on the box, `pt` env:
...
**§7 carries this either way**: the deployed conservative reward's tail selection
was driven substantially by an unidentified direction that the gauge discards,
and the method was corrected before the reported round.
...
> ❌ **RETRACTED 2026-09-22 (§4.3.126 §5) — this rested on n=4 and did not
> survive a fifth trial.** The median `|log r|` at depth 4 fell from **1.6434 to
> 0.2838**, so the "18× jump" is **3.8×** and the *cliff* framing is withdrawn.
> The monotone **gradient** survives (eligibility 83% / 69% / 67% / 40% at
> d1–d4), and that is the defensible claim. The methodological aside below —
> that a rank correlation is the wrong summary — was built on the artefact and
> goes with it. Quote any depth-4 statement with its n; the cell is still only 5.
...
### 4.3.126 ROUND 5 at 81 trials — large_play STOPPED, medium_diverse BROKE the width lock-in, and §4.3.121's depth cliff was an n=4 artefact

Checked 2026-09-22 (second check that day). 81 finished (21/19/20/21).

#### 1. large_play's stopping rule FIRED at trial 20 — cleanly

**Winner `q45qbz8h`, width 5 × depth 2, 2,305p, J = 0.470127.** The rule winner,
the best-of-all and the **eligible** best are all the same trial, so §3.2.7's
ungated-vs-eligible split — the round-2 failure §7.2 records — **did not
materialise here**. Nothing is owed on that account.

All four gates are comfortably clear: `|log r|` 0.0122 (**4.6σ** inside),
`loc_sd` 0.1124 (**3.6σ**), margin +0.1137 (**31σ**), ess 71.8. §4.3.108's
"eligibility is seed-dependent within ~1σ" disclosure does **not** apply.

> ⚠️ **The sweep is still RUNNING and its remaining trials are discardable.**
> The rule fired at trial 20; anything after it is outside the pre-registered
> budget. **Stop the agent** — it frees a GPU, and §10.7 measured 4 concurrent
> sweeps costing **2.8× throughput**, so the other three speed up.

#### 2. …but large_play sampled depth 1 only TWICE, and both failed

| lp depth-1 trial | w | n_params | `\|log r\|` | `loc_sd` | verdict |
|---|---|---|---|---|---|
| 6 `c6zuq01h` | 5 | 1,249 | 0.3399 | **0.2943** | FAIL scale **and** loc |
| 15 `5ueev7qz` | 7 | 4,993 | 0.2512 | 0.0999 | FAIL scale |

Its depth histogram is **{1: 2, 2: 6, 3: 10, 4: 2}** — it sampled depth 3 ten
times and depth 1 twice, both times with step sizes that failed gate 1, and then
avoided the region. **Depth 1 supplies 3 of the other 4 leaders and is 83%
eligible overall.**

> **This is a DISCLOSURE, not a reason to re-run.** The stopping rule is
> pre-registered and it fired; re-running large_play because its coverage looks
> unlucky is precisely the reactive tuning §0 forbids — the same trap as the
> medium_diverse replica (§4.3.125 §4). **Report in §7**: large_play's winner was
> selected from 20 trials in which the depth-1 region was sampled twice, both
> times failing gate 1 on the sampler rather than on the architecture, while that
> region supplies the other variants' winners.
>
> ⚠️ **Narrowed 2026-09-24 (§7.4 B).** "Failing on the sampler rather than on the
> architecture" overclaims from two trials. When checked, both depth-1 trials sat
> outside large_play's eligible step-size band (trial 15 on 3 of 3 settings,
> trial 6 on 1 of 3, by 16%). But being in band is not sufficient to pass:
> medium_play has an in-band depth-1 trial that failed. The supportable claim is
> **"depth 1 was never tested under settings that work for large_play"**. §7.4 B
> has the wording.

#### 3. medium_diverse BROKE the width lock-in — at trial 19, exactly as the mechanism predicts

| md trials | width | depth | J | plateau? |
|---|---|---|---|---|
| 1–12 | **7** | 2–4 | 6 of 12 at **exactly `log 2`** | **yes — GP has no gradient** |
| 13–17 | 7 | **1** | 0.4707 / 0.5180 / **0.4601** / 0.4685 / 0.4639 | no — informative |
| 18 | 7 | 1 | 0.6407 (fails scale) | no |
| **19** | **6** | 1 | **0.4227 — new leader** | no |

**The last plateau trial was 12. Six informative trials later, at trial 19, the
GP left width 7 for the first time in the sweep's history — and immediately
improved**, J 0.4601 → **0.4227** (−8%). Its leader is now **w6 × d1, 2,497p** —
the *same architecture* as medium_play's leader.

> ✅ **§4.3.125 §3's mechanism is now confirmed inside the original sweep, with
> no replica needed.** Item C's `J → log 2` saturation removed the gradient; the
> sweep froze on its opening width. Restore the gradient — which depth 1 did, by
> producing eligible trials — and exploration resumes within a handful of trials.
> **The lock was temporary and mechanistic, not a permanent search failure.**
>
> It did cost something real: 18 trials at a width that was **8% worse** than the
> one next door. That is the number §7 should quote for item C's side effect.

#### 4. 🔽 The replication probe is DOWNGRADED to optional

§4.3.125 §4 queued 2–3 medium_diverse replicas to test whether the lock-in
reproduces. **Its main motivation is gone**: the question was whether the lock-in
was chance or landscape, and the answer is now *neither* — it was item C's
plateau, demonstrated by the sweep un-locking itself once the plateau ended.

What a replica would still add is narrow: whether the **early plateau** itself
reproduces, which bears only on a §7 sentence about search-procedure variance.
**The pre-registered design and the non-selection constraint in §4.3.125 §4 stand
unchanged if it is run** — but it is no longer needed to explain anything, and
the GPU is better spent elsewhere.

#### 5. ❌ CORRECTION — §4.3.121's "depth 4 is a cliff, not a slope" was an n=4 artefact

§4.3.121 §3 claimed "an **18×** jump in median `|log r|` between d3 and d4" and
made a methodological point out of it ("a rank correlation is the wrong summary
for a cliff"). **One additional depth-4 trial changed it:**

| | §4.3.121 (n=4 at d4) | **now (n=5 at d4)** |
|---|---|---|
| median `\|log r\|` at d4 | **1.6434** | **0.2838** |
| ratio to d3 | 18× | **3.8×** |
| eligible at d4 | 1/4 (25%) | 2/5 (40%) |

**The monotone gradient survives — eligibility 83% / 69% / 67% / 40% across d1–d4
— but the "cliff" framing does not.** It rested on four trials, and the fifth cut
the median by 83%.

> **This is §4.3.108's lesson landing on my own analysis**: mid-sweep statistics
> on small cells move a lot, and I over-read a 4-trial cell into a mechanism.
> The defensible claim is the **gradient**, which is stable across 38, 65 and 81
> trials. **Any depth-4 statement should be quoted with its n**, which is still
> only 5 — the optimiser avoids d4, so that cell will stay thin.

#### 6. Current state

| sweep | trials | eligible | leader | w × d | n_params | J |
|---|---|---|---|---|---|---|
| medium_play | 21 | 16 (76%) | `9jcqok4g` | 6 × 1 | 2,497 | **0.3767** |
| medium_diverse | 19 | 7 (37%) | `kk0128c1` | 6 × 1 | 2,497 | 0.4227 |
| **large_play** | **20 — STOPPED** | 15 (75%) | `q45qbz8h` | 5 × 2 | 2,305 | 0.4701 |
| large_diverse | 21 | 20 (95%) | `owlrd69d` | 4 × 1 | 625 | 0.3980 |

Gate 1 scale remains the only binding constraint. medium_play is at 5/15
...
> ❌ **Did not hold (2026-09-24, §7.4 A).** Three trials later `jd9pf1eh` landed
> at 0.4055, **0.05×** the joint 2·SE from the winner, so medium_diverse is tied
> like the other three. This section's own caution ("1.2× is a narrow margin")
> was the right reading. On a landscape this flat a lone separated winner is
> transient.
...
ran, so none of it is post-hoc. §7.4 covers the **round-5** results — the round
that selects the reported BNN winners — and its provisional numbers are
regenerated by `scripts_bnn/round5_disclosures.py`. §7.5 covers the two
**procedure changes of 2026-09-15** (common capacity ranges; MR/PT test-split
checkpoint selection) under which every reported winner was selected.
...
**A. Every BNN winner is statistically tied with its runners-up — the selected
TRIAL is arbitrary, the selected ARCHITECTURE is not.** (to-do 18i)

The winner rule (§3.2.12) takes the lowest `val_cvar_ce` among eligible trials.
Treating two trials as indistinguishable when
`|ΔCE| ≤ 2·√(SE_winner² + SE_trial²)` gives:
...
No winner is separated from its runner-up. medium_diverse briefly had one: at 27
trials its runner-up sat 1.2× the joint 2·SE away (§4.3.128). Three trials later
a new trial landed 0.05× away. **On a landscape this flat, a lone separated
winner is transient.**

**What this means, stated so it cannot be read as more or less than it is:**

- **The specific trial is an argmin over noise.** The winner's sampler settings
...
- **This holds on the selection objective only.** The tied trials match on
  ...
  would give the same policy". What the data supports is "any member would have
  been selected with equal justification".

**§3.2.5's design target failed, and not for the reason it guarded against.**
...
**The comparison with the baselines is asymmetric, and a reader must be told
so.** MR and PT select on `eval_loss_at_selected`, which **logs no standard
error**, verified on both families' sweeps. The same tie analysis **cannot be
run** on them. Nothing here shows that BNN selection is noisier than baseline
selection. It shows that the BNN is the only family whose selection noise was
*measured*. The likelier reading is that the baselines' winners are also
tied with their neighbours and nobody can see it. State the asymmetry rather
than letting "the BNN winner is statistically tied" stand alone.

**One mechanical consequence, held as a watched threshold (to-do 18j).** Of the
improvements that reset the K=15 counter, several were smaller than the
...
**B. large_play's search never tested depth 1 under sampler settings that work
for large_play.** (to-do 18h — **final**; the sweep stopped at trial 20)

large_play's winner (w5×d2, 2,305p) was selected from 20 trials with depth
histogram **{1: 2, 2: 6, 3: 10, 4: 2}**. Both depth-1 trials failed gate 1. But
every one of large_play's **15 eligible trials** has `sghmc_lr` inside
**5.2e-05 – 9.0e-05**, the bottom quarter-decade of the searched 5e-05 – 5e-04.
**No other variant is this narrow**: medium_play and large_diverse are eligible
across most of the decade. The two depth-1 trials both sat outside that band:

| depth-1 trial | `sghmc_lr` | `sghmc_lr_max` | `mdecay` | outside large_play's eligible range on |
|---|---|---|---|---|
...
- **Depth 1 was never tested fairly on large_play.** This is the claim the data
  supports.
- **It cannot be concluded that depth 1 would have won, or even passed.** A
  sampler setting inside the eligible band is not enough to pass. medium_play has
  a depth-1 trial (`e34c4ye6`) that sat inside its eligible range on all three
  settings and still failed. The band is also estimated from large_play's
  depth-2–4 trials only, and step size and depth may interact. The evidence for
  trial 6 is weak (1 of 3 settings, by 16%). It is strong only for trial 15.
- **This is not grounds to re-run large_play.** The stopping rule is
  pre-registered and it fired (§3.1, §0). Re-running a sweep because its coverage
  looks unlucky after the result is known is reactive tuning. It is the same
  trap as the medium_diverse replica (§4.3.125 §4).

**Suggested wording for the paper:** *large_play's search sampled depth 1 twice,
both times with step sizes outside the narrow range in which large_play trained
successfully at any depth. Depth 1 therefore went effectively untested for this
variant, although it produced the selected architecture for the other three. We
report the selected depth-2 configuration as found and make no claim about
depth 1 for large_play.*
...
**D. The penalised objective (item C) froze one sweep's search for 18 trials.**
(§4.3.125 §3, §4.3.126 §3 — **final**; the episode is over)

Round 5's optimiser minimises `J = cvar_ce + (1 − P)·max(0, log 2 − cvar_ce)`,
where `P` is the probability of passing all four gates (§3.2.17). The design
goal was to make ineligible trials unattractive, and it worked: gate 2 no longer
binds, and eligibility went from 15% of round-4 trials to 66% of round 5's first
65 (§4.3.125).

**It has a side effect that has to be reported.** When a trial is far from
eligible, its gate slacks are ~6 standard deviations out, `P` underflows to 0,
and `J` collapses to **exactly `log 2`**, however the trial actually performed.
Such trials become indistinguishable to the Bayesian optimiser. It sees a flat
surface with no gradient in any direction.

This happened to one sweep:

| | trials at `J = log 2` exactly | distinct `J` values | left its opening width at |
|---|---|---|---|
| **medium_diverse** | **6 of its first 12** (5 of the first 8) | 10 of 15 | **trial 19** |
| medium_play / large_play / large_diverse | 1 / 2 / 0 | all or all-but-one | trial 2, all three |

medium_diverse drew width 7 at trial 1, a 1-in-4 chance. Its early trials sat at
depths 2–4, where it fails gate 1, so they fell on the plateau, and the optimiser
never moved off width 7. **The lock broke by itself.** Once depth-1 trials began
producing informative `J` values (trial 13), exploration resumed six trials after
the last plateau trial. The first move, to width 6 at trial 19, improved `J`
from 0.4601 to **0.4227**, a gap of **8.3× the objective's resolution**. The
sweep then walked down to its current w5×d1 leader.

**What to report:**

- **The cost was search budget, not the result.** 18 trials were spent at a
  width that turned out 8% worse than its neighbour. The sweep still found an
  eligible winner, and nothing indicates the final selection is affected.
- **It is a property of the objective's form, not of width 7 or of chance
  alone.** The same opening draw in a sweep without the plateau would have been
  left by trial 2, as the other three show. The penalty is informative near the
  eligibility boundary and uninformative far from it, because `P` saturates.
- **It interacts with the trial-count disclosure (A, 18j).** The locked trials
  count toward medium_diverse's budget, which is one reason that sweep may run
  longer than the others.
- **Not changed mid-campaign.** Item C is pre-registered (§3.2.17), §0 forbids
  reactive tuning, and the affected sweep recovered on its own. A form that keeps
  a gradient far from the boundary, e.g. penalising on `log P` rather than
...
the MR/PT baselines and round 5 of the BNN. **Every winner the paper reports was
selected under them.** The third redesign change of that date, stage 4's last-10
statistic, is already disclosed in §7.1.

---

**A. The capacity ranges were replaced with common ranges after results had been
seen — a declared deviation from §9.** (§3.2.16)

| family | before | **after (reported)** |
|---|---|---|
| MR, BNN | `width` log2 6–9 × `depth` 1–6 — 2,497 to 1,333,249 params | **`width` log2 4–7 × `depth` 1–4 — 625 to 54,529** |
| PT | `embd_dim` 6–8 × `head_dim` 5–7 × layers 1–4 — 67,521 to 3,327,489 | **`embd_dim` 3–5 × `head_dim` 3–5 (clamped) × layers 1–4 — 2,169 to 57,537** |

The new ranges are **identical across variants** and **matched across
families**. All share depth 1–4 and a ~55k-parameter ceiling. PT's floor stays
~3× the MLP floor, which is irreducible transformer overhead.

**What must be disclosed, in §3.2.16's own terms:** §9 forbids changing a search
range in response to observed sweep behaviour. **This change was informed by
observed behaviour**, namely the capacity ladders, the round-3 BNN trials and the
stage-1 baselines. **So it deviates from §9 in letter.** It followed §9's
prescribed remedy:

- every sweep (4 BNN, 4 MR, 4 PT) was **restarted from scratch** under new id
  caches, so no trial selected under the old ranges survives;
- the ranges are **uniform** across families and variants, not tailored to any
  one of them;
- the grounds are **general**: labels per parameter, the interpolation
  threshold, HMC mixing cost growing with dimension, a low-dimensional reward.
  The observed results were used **in pooled form only**.

**How it came out, which the paper should state beside the deviation:**

- **It did not tilt the ranges toward the BNN.** The ceiling binds the
  *baselines* in 6 of 8 sweeps (§7.1). The BNN's four winners all sit at
  **625–2,497 params**, the bottom decade of the range (§4.3.128), and its
  eligible trials span the whole range, so the ceiling did not bind the BNN.
- **One of its stated grounds is partly refuted.** It discounted MR's preference
  for large models as an artefact of the old checkpoint rule. That preference
  persists under the new rule (§7.1). The ranges stand on their other grounds.

---

**B. MR/PT checkpoint selection moved from the validation split to the test
split, and this changed the winner in every baseline sweep.** (§4.3.107)

**What changed.** An MR or PT run trains all 5,000 epochs and saves one
checkpoint for IQL to load. Before, **validation loss chose that checkpoint and
also ranked the hyperparameters**: the sweep objective was the minimum
validation loss over epochs, so the same split was used twice and the objective
was optimistically biased. Now **the test split chooses the checkpoint, and
validation scores it** (`eval_loss_at_selected`). The two splits are the same
size, so no data is lost.

**Scope.** It applies to MR best-model and PT, which load the selected checkpoint.
The MR ensemble's IQL reward averages every snapshot past burn-in and does not
use the selection, although it shares the MR sweep objective. **The BNN selects
no checkpoint and is unaffected.** Every earlier MR/PT stage-1 winner, stage-4
grid and seeds 1–10 reward model was trained under the old rule and is re-run.

**Measured on the completed sweeps: all 283 finished runs, all 8 sweeps**
(`scripts_bnn/split_role_readout.py`, output `exp/split_role_readout.txt`):

| family | variant | n | median optimism removed | pairs reordered | winner under the old objective ranks, under the new |
|---|---|---|---|---|---|
| MR | medium_play | 27 | +0.0063 | 17.9% | #9 of 27 |
| MR | medium_diverse | 64 | +0.0053 | 36.2% | **#62 of 64** |
| MR | large_play | 34 | +0.0152 | 21.7% | #10 of 34 |
| MR | large_diverse | 26 | +0.0000 | 20.0% | #22 of 26 |
| PT | medium_play | 66 | +0.0488 | 46.2% | **#61 of 66** |
| PT | medium_diverse | 29 | +0.0429 | 29.3% | #14 of 29 |
| PT | large_play | 20 | **+0.1497** | 47.9% | #17 of 20 |
| PT | large_diverse | 17 | +0.0457 | 35.3% | #5 of 17 |

- **The winner changes in all 8 sweeps.** In two of them the configuration the
  old rule would have picked ranks **near the bottom** on the honest objective.
- **PT was flattered far more than MR.** Median optimism is 0.043–0.150 for PT
  against 0.000–0.015 for MR, consistent with §4.3.106's PT val/test divergence.
- **18–48% of trial pairs change order**, so the change alters the ranking, not
  just the reported level.

**How to read the table, without over-reading it.** These are same-run
counterfactuals. For the trials actually run, it asks which one each objective
would have selected. It does **not** show what an old-objective sweep would have
explored. The Bayesian optimiser was steered by the new objective, so the trial
set was shaped by it too. It also does not compare against the round-1 winners,
which came from different ranges. It establishes that **the choice of objective
decides the winner on these runs**. It does not establish how much the old rule
cost in IQL return.

**What it costs, and which way it points:**

- **There is no longer a held-out test loss for MR/PT**, because test is now used
  to choose the checkpoint. Reward-model losses are fit statistics and are not
  what the paper evaluates (IQL return is), so this is a small cost. It should
  still be stated, so no reader looks for a test-loss column.
- **It does not flatter the BNN.** It removes a double use of validation data
  from the *baselines'* selection, which should if anything improve the
  baselines' selected models. It leaves the BNN's procedure untouched.
- **§4.3.108's figures are superseded.** They came from a 10-trial subsample of 6
  of the 8 sweeps, taken mid-sweep. The full data **agree** with them: PT is
  flattered most, and the reordering is 18–48% against 22–49%. What the full
  data add is the winner change, **measured directly** in all 8 sweeps where
  §4.3.108 had inferred it from pair flips, and the two missing PT sweeps.
...
- **The `n_meas` ladder ran and is read out** (§4.3.112 RESULT): **DOES NOT
  ...
  produced.
- **DECIDED: centring goes into BOTH paths — bundle item F** (§4.3.114). The
...
3. ✅ **DONE — the `n_meas` ladder ran and did not fire** (§4.3.112 RESULT).
   ...
   *(superseded text)* **Run the `n_meas` ladder — §4.3.112.** Six
   ...
   established.
4. **Run the free centred-objective check** (§4.3.114) — ten runs with saved
...
16. ✅ **DONE** — the corrected derived `map_amp2` (6848 / 6838) is in §7.3's
...
17. ✅ **BUILT AND VERIFIED (§4.3.124)** — `reward_label_cache.py` (self-test
    ...
    biases nothing.
18. ✅ **DONE 2026-09-24** — all three owed procedure disclosures are written.
    **§3.2.16's declared §9 amendment** is §7.5 A. **The MR/PT split-role
    change** is §7.5 B, recomputed on all 283 finished runs by
    `scripts_bnn/split_role_readout.py`: **the winner changes in 8 of 8
    sweeps**, and in two the old rule's pick ranks #62/64 and #61/66 on the
    honest objective. **The last-10 statistic** was already in §7.1. The round-5
    *results* live in §7.4.
...
18f. ✅ **DONE — large_play STOPPED at trial 20** (§4.3.126). Winner `q45qbz8h`
    (w5×d2, J=0.4701); rule winner, best-of-all and **eligible** best all
...
18l. ✅ **18h and 18i WRITTEN as §7.4 A and B** (2026-09-24), backed by
    ...
    final.** The claim was *narrowed* after checking. It is not "failed on the
    sampler" but "**never tested depth 1 inside large_play's working step-size
    band**" (5.2e-05–9.0e-05, the bottom quarter-decade, unique to that variant).
    Being in band is not sufficient to pass. **A (tied winners) is final for the
    large variants and provisional for the medium ones.** It adds the
    **asymmetry**: MR/PT log no SE, verified, so they cannot be checked the same
    way. **When medium_play and medium_diverse stop, rerun the readout and fill
    §7.4 C.**
18i. ✅ **DONE — written as §7.4 A** (2026-09-24; see 18l). Final for the large
    variants, provisional for the medium ones until they stop. Original
    brief, kept as history: **§7 disclosure owed: the winners are statistically TIED with their
...
18h. ✅ **DONE — written as §7.4 B** (2026-09-24; see 18l). Final. The claim was
    narrowed when written: "on the sampler" became "never tested inside
    large_play's working step-size band". Original brief, kept as history:
    **§7 disclosure owed for large_play's coverage** (§4.3.126 §2): it sampled
    **depth 1 twice in 20 trials and both failed gate 1 on the sampler**, while
    ...
    disclosure, **not** a reason to re-run — the rule is pre-registered and fired.
18g. 🔽 **OPTIONAL (was queued): the medium_diverse replication probe.** Its
    motivation is largely gone — medium_diverse **broke its own width lock-in at
    trial 19** and the mechanism is confirmed inside the original sweep
    (§4.3.126 §3). If run anyway, the design and the **must-not-supply-a-winner**
    constraint in §4.3.125 §4 stand unchanged.
