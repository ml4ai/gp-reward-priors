...
class MapperWassersteinBNN(object):
    def __init__(
        self,
        trained_bnn,
        bnn,
    ...
    def optimize(
        ...
        def compute_sqw2(X):
            ...
            evalues, evectors = torch.linalg.eigh(sqrt_target_K @ bnn_K @ sqrt_target_K)
            sqrt_evalues = torch.sqrt(torch.clamp(evalues, min=0.0))
            fidelity = (
                evectors @ torch.diag_embed(sqrt_evalues) @ evectors.transpose(-2, -1)
            )
