...
from optbnn.prior_mappers.wasserstein_bnn_mapper import MapperWassersteinBNN
...
class TrainConfig:
    ...
    prior_dir: str = "./exp/reward_learning/bb_tuning_star/br-02336596"
    prior_ckpt: int = 300
    bnn_dir: str = "./exp/reward_learning/bb_optim/br-bb_t0012-b8ced45f"
    ...
    def __post_init__(self):
        ...
        self.prior_dir = os.path.join(
            ...
            "ckpts",
            "it-{}.ckpt".format(self.prior_ckpt),
        )

        self.bnn_dir = os.path.join(osp.expanduser(self.bnn_dir), "sampling_optim")
...
def train(config: TrainConfig):
    ...
    bnn_width = config.bnn_width
    bnn_depth = config.bnn_depth
    bnn_transfer_fn = "relu"

    bnn_prior = OptimGaussianPrior(config.prior_dir)
    bnn_net = MLP(28, 1, [bnn_width] * bnn_depth, bnn_transfer_fn)
    bnn_likelihood = LikCE()
    bnn_bayes_net = PrefNet(bnn_net, bnn_likelihood, bnn_prior, config.bnn_dir, n_gpu=1)
    bnn_bayes_net.sampled_weights = bnn_bayes_net._load_sampled_weights(
        os.path.join(config.bnn_dir, "sampled_weights", config.weights_dir)
    )
    ...
    with h5py.File(config.tuning_set) as f:
        data_generator = DataSetSampler(f["obs"][:])
