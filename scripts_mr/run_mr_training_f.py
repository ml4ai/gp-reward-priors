...
class TrainConfig:
    ...
    epochs: int = 10
    batch_size: int = 256  # Batch size for training/test data loaders
    lr: float = 3e-4
    eval_every: int = 1  # How often (epochs) to run evaluation
...
def train(config: TrainConfig):
    ...
    dataset = util.Pref_H5Dataset(dataset_path, -1)
    ...
    for epoch in range(config.epochs + 1):
        ...
        if epoch:
            ...
            # Epoch 0: baseline eval only; log nan for training metrics.
